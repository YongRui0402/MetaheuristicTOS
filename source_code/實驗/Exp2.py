...
for parmi in range(len(parm)):
    ...
    for testi in range(test_time):
        ...
        for i in range(boardstateListNum):
            ...
            rcidfs = RCIDFS(boardstring,find_max_sol,limitpathlen=limitpathlen,limittime=limittime_list[method],seed_value=seed_values[testi])
        ...
        savepath = f"./實驗結果/{dataset}/{method_str}_{testi}_{parmi}.json"
        result.savejson(savepath)
        
        print(f"演算法{method_str},平均分數:{meanScroce},平均量化分數:{meanScroceR},平均花費時間:{meantime},平均路進長度:{meanPathlen}")
    MmeanScroce /= test_time
    MmeanScroceR /= test_time
    Mmeantime /= test_time
    MmeanPathlen /= test_time
...
for parmi in range(len(parm)):
    ...
    for testi in range(test_time):
        ...
        for i in range(boardstateListNum):
            ...
            ga = GA(boardstring,limitpathlen=limitpathlen,limittime=limittime_list[method],seed_value=seed_values[testi])
...
for parmi in range(len(parm)):
    ...
    for testi in range(test_time):
        ...
        for i in range(boardstateListNum):
            ...
            mmas = MMAS(boardstring,ants_num,update_step,limitpathlen=limitpathlen,limittime=limittime_list[method],seed_value=seed_values[testi])
